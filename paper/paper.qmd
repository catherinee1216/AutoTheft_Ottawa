...
# Merging 'theft_recovery' dataframe with 'auto_Data'
merged_data <- merge(auto_Data, theft_recovery, by = "theft_year", all.x = TRUE)

# Plotting the merged data as a line graph
ggplot(merged_data, aes(x = theft_year, y = recovered)) +
  geom_line() + 
  geom_point() +
  labs(x = "Year", y = "Count") +
  theme_minimal()
...
years_vehicle <- theft_recovery |>
  mutate(vehicle_year = as.numeric(neighbourhood))  # Convert to numeric if 'vehicle_year' is factor

# Create a scatter plot
ggplot(years_vehicle, aes(x = neighbourhood, y = n, fill = neighbourhood)) +
  geom_point() +  
  theme_minimal() + 
  theme(legend.position = "bottom") + 
  labs(x = "Vehicle Brand", y = "Count", fill = "Count", title = "Top 10 Brands with Most Auto Thefts in 2022")
